...
public class AuthServiceApplication {
    ...
    CommandLineRunner demo(AccountRepository accountRepository) {
        return args -> {
            Stream.of("chaoluo,password", "test,test")
                    .map(tpl -> tpl.split(","))
...
class AccountDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        return accountRepository.findByUsername(username)
                .map(account ->
                    new User(account.getUsername(), account.getPassword(), account.isActive(), account.isActive(), account.isActive(), account.isActive()
                    , AuthorityUtils.createAuthorityList("ROLE_ADMIN"))
                ).orElseThrow(() -> new UsernameNotFoundException("cloud"));
    }
}
